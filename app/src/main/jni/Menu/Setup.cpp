...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle) {
    //Set sub title
    setText(env, title, OBFUSCATE("<b>VortexHacker</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p>Modded by VortexHacker</p> | "
                                     "https://github.com/VortexDev1 | Whatever Game V1</p>"));

    //Dialog Example
    //setDialog(ctx,env,OBFUSCATE("Title"),OBFUSCATE("Message Example"));

    //Toast Example
    Toast(env, ctx, OBFUSCATE("Credits: VortexHacker"), ToastLength::LENGTH_LONG);
}
...
int RegisterMenu(JNIEnv *env) {
    ...
    jclass clazz = env->FindClass(OBFUSCATE("com/vortex/lgl/Menu"));
    if (!clazz)
        return JNI_ERR;
...
int RegisterPreferences(JNIEnv *env) {
    ...
    jclass clazz = env->FindClass(OBFUSCATE("com/vortex/lgl/Preferences"));
    if (!clazz)
        return JNI_ERR;
    if (env->RegisterNatives(clazz, methods, sizeof(methods) / sizeof(methods[0])) != 0)
        return JNI_ERR;
...
int RegisterMain(JNIEnv *env) {
    ...
    jclass clazz = env->FindClass(OBFUSCATE("com/vortex/lgl/Main"));
    if (!clazz)
        return JNI_ERR;
    if (env->RegisterNatives(clazz, methods, sizeof(methods) / sizeof(methods[0])) != 0)
        return JNI_ERR;
